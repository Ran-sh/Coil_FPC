function varargout = Via_Planner(operation, varargin)
%VIA_PLANNER Series and output-via placement.
    ...
    if rowSpan > 2*innerClearHalfL + tol
        ...
        failureReason = sprintf( ...
            ['内圈空白区域不足以横向排列 %d 个过孔：所需长度 %.3f mm，', ...
             '可用长度 %.3f mm。建议减小内圈过孔数量、增大板尺寸或调整自动过孔参数（innerViaPitch）。'], ...
...
if outerCount > 0
    ...
    if ~okTab
        ...
        if strcmp(failureCode, 'TAB_VIA_CAPACITY')
            % 该代码有多个来源：X 方向长度预检，以及 Y 方向候选被 PAD / VOUT /
            % L1 回路线净距（或 6/8 层的反焊盘引线通道下界）耗尽。消息必须
            % 覆盖全部来源，不能只报长度。
            if cfg.layerCount > 4
                failureReason = sprintf( ...
                    ['无法在右侧尾板放置 %d 个层间过孔：可用空间不足、与 PAD / VOUT / 回路线净距冲突，', ...
                     '或 6/8 层贯穿通孔模型的反焊盘引线通道容不下 %.3f mm 的行偏移下限', ...
                     '（请求 %.3f mm，方向 %s）。', ...
                ...
                failureReason = sprintf( ...
                    ['无法在右侧尾板放置 %d 个层间过孔：可用空间不足，或与 PAD / VOUT / 回路线净距冲突。', ...
                     '建议增大 tabLength / tabWidth、减小过孔数量，或调整 outerViaPitch / outerViaRowOffsetY。'], ...
                    outerCount);
            end
        end

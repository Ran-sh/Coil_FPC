function varargout = LeadRouter(operation, varargin)
...
if RectangularFpc.Geometry.PathGeometry('candidate_compliant', basePath, direct, mode, boardXY, cfg)
    path = direct;
    ok = true;
    return;
end

[filletPath, filletOk] = RectangularFpc.Geometry.LeadRouter('smooth_lead',  ...
    startPt, startTangent, endPt, bendRadius, arcPointCount, tol);
if filletOk && RectangularFpc.Geometry.PathGeometry('candidate_compliant', basePath, filletPath, mode, boardXY, cfg)
    path = filletPath;
    ok = true;
    return;
end
...
for wpIndex = 1:numel(waypoints)
    ...
    if ~RectangularFpc.Geometry.PathGeometry('candidate_compliant', basePath, cand, mode, boardXY, cfg)
        continue;
    end
    candLen = RectangularFpc.Geometry.PathGeometry('path_length', cand);
...
for priority = 1:3
    ...
    for k = 1:numel(candidateIndices)
        lengths(k) = RectangularFpc.Geometry.PathGeometry('path_length', candidates{candidateIndices(k)});
    end
...
[path1, ok1] = RectangularFpc.Geometry.LeadRouter('smooth_lead',  ...
...
[path2, ok2] = RectangularFpc.Geometry.LeadRouter('smooth_lead',  ...

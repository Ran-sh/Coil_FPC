function varargout = CoilSpiral(operation, varargin)
...
d.outerPerimeter = RectangularFpc.Geometry.BoardOutline('perimeter',  ...
...
if strcmp(cfg.outputViaPlacementMode, 'manual')
    d.outputVia = RectangularFpc.Geometry.PathGeometry('user_to_internal', cfg.manualOutputViaXY, cfg);
else
    d.outputVia = RectangularFpc.Geometry.ViaPlanner('auto_output_via', cfg);
end
...
for k = 1:cfg.layerCount
    ...
    rawLayerXY{k} = RectangularFpc.Geometry.CoilSpiral('spiral', cfg, limits, direction);
end
...
[vias, viaFailCode, viaFailReason] = RectangularFpc.Geometry.ViaPlanner('plan_vias', cfg, spiralInfo);
...
[keepoutXY, keepoutRadii] = RectangularFpc.Geometry.LeadRouter('through_keepouts', vias, 1, cfg);
[lead, okLead] = RectangularFpc.Geometry.LeadRouter('orthogonal',  ...
...
for k = 1:cfg.layerCount-1
    ...
    [fromKeepoutXY, fromKeepoutRadii] = ...
        RectangularFpc.Geometry.LeadRouter('through_keepouts', vias, k, cfg);
    [toKeepoutXY, toKeepoutRadii] = ...
        RectangularFpc.Geometry.LeadRouter('through_keepouts', vias, k + 1, cfg);
    [lead1, ok1] = RectangularFpc.Geometry.LeadRouter('orthogonal',  ...
        rawLayerXY{k}, vias(k).xy, 'append', ...
        bendRadius, cfg.leadArcPointCount, boardXY, cfg, ...
        fromKeepoutXY, fromKeepoutRadii);
    [lead2raw, ok2] = RectangularFpc.Geometry.LeadRouter('orthogonal',  ...
    ...
    vias(k).fromLeadLength = RectangularFpc.Geometry.PathGeometry('path_length', lead);
    if ~isempty(lead) && size(lead, 2) == 2
        layerXY{k} = [layerXY{k}; lead(2:end,:)];
    end
    leadRev = flipud(lead2raw);
    vias(k).toLeadPath = leadRev;
    vias(k).toLeadLength = RectangularFpc.Geometry.PathGeometry('path_length', leadRev);
...
if strcmp(cfg.outputViaPlacementMode, 'manual')
    ...
    [outputViaInside, centerToBoard, requiredDistance] = ...
        RectangularFpc.Geometry.ViaPlanner('validate_board_location', d.outputVia, cfg.viaPadDiameter, ...
...
[keepoutXY, keepoutRadii] = ...
    RectangularFpc.Geometry.LeadRouter('through_keepouts', vias, cfg.layerCount, cfg);
[lead, okLead] = RectangularFpc.Geometry.LeadRouter('orthogonal',  ...
...
vout.fromLeadLength = RectangularFpc.Geometry.PathGeometry('path_length', lead);
if ~isempty(lead)
...
% ---- 4) 整理与连接误差 ----
for k = 1:cfg.layerCount
    layerXY{k} = RectangularFpc.Geometry.PathGeometry('remove_duplicates', layerXY{k}, tol);
    layerXY{k} = RectangularFpc.Geometry.PathGeometry('remove_zero_length', layerXY{k}, tol);
end

function varargout = Coil_Spiral(operation, varargin)
%COIL_SPIRAL Spiral construction and layer assembly.
...
d.outerPerimeter = RectangularFpc.Geometry.Board_Outline('perimeter',  ...
...
if strcmp(cfg.outputViaPlacementMode, 'manual')
    d.outputVia = RectangularFpc.Geometry.Path_Geometry('user_to_internal', cfg.manualOutputViaXY, cfg);
else
    d.outputVia = RectangularFpc.Geometry.Via_Planner('auto_output_via', cfg);
end
...
for k = 1:cfg.layerCount
    ...
    rawLayerXY{k} = RectangularFpc.Geometry.Coil_Spiral('spiral', cfg, limits, direction);
end
...
[vias, viaFailCode, viaFailReason] = RectangularFpc.Geometry.Via_Planner('plan_vias', cfg, spiralInfo);
...
[keepoutXY, keepoutRadii] = RectangularFpc.Geometry.Lead_Router('through_keepouts', vias, 1, cfg);
[lead, okLead] = RectangularFpc.Geometry.Lead_Router('orthogonal',  ...
...
for k = 1:cfg.layerCount-1
    ...
    [fromKeepoutXY, fromKeepoutRadii] = ...
        RectangularFpc.Geometry.Lead_Router('through_keepouts', vias, k, cfg);
    [toKeepoutXY, toKeepoutRadii] = ...
        RectangularFpc.Geometry.Lead_Router('through_keepouts', vias, k + 1, cfg);
    [lead1, ok1] = RectangularFpc.Geometry.Lead_Router('orthogonal',  ...
        rawLayerXY{k}, vias(k).xy, 'append', ...
        bendRadius, cfg.leadArcPointCount, boardXY, cfg, ...
        fromKeepoutXY, fromKeepoutRadii);
    [lead2raw, ok2] = RectangularFpc.Geometry.Lead_Router('orthogonal',  ...
    ...
    vias(k).fromLeadLength = RectangularFpc.Geometry.Path_Geometry('path_length', lead);
    if ~isempty(lead) && size(lead, 2) == 2
        layerXY{k} = [layerXY{k}; lead(2:end,:)];
    end
    leadRev = flipud(lead2raw);
    vias(k).toLeadPath = leadRev;
    vias(k).toLeadLength = RectangularFpc.Geometry.Path_Geometry('path_length', leadRev);
...
if strcmp(cfg.outputViaPlacementMode, 'manual')
    ...
    [outputViaInside, centerToBoard, requiredDistance] = ...
        RectangularFpc.Geometry.Via_Planner('validate_board_location', d.outputVia, cfg.viaPadDiameter, ...
...
[keepoutXY, keepoutRadii] = ...
    RectangularFpc.Geometry.Lead_Router('through_keepouts', vias, cfg.layerCount, cfg);
[lead, okLead] = RectangularFpc.Geometry.Lead_Router('orthogonal',  ...
...
vout.fromLeadLength = RectangularFpc.Geometry.Path_Geometry('path_length', lead);
if ~isempty(lead)
...
% ---- 4) 整理与连接误差 ----
for k = 1:cfg.layerCount
    layerXY{k} = RectangularFpc.Geometry.Path_Geometry('remove_duplicates', layerXY{k}, tol);
    layerXY{k} = RectangularFpc.Geometry.Path_Geometry('remove_zero_length', layerXY{k}, tol);
end

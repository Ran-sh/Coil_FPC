function Export_Verification(operation, varargin)
%EXPORT_VERIFICATION Read back the complete staged artifact set.
...
RectangularFpc.Export.Dxf_Artifacts('verify_all', outputFolder, cfg, result);
...
expectedSvgNames = RectangularFpc.Export.Preview_Export('expected_names', cfg);
...
internalXY = [result.pads(1).xy; reshape([result.vias.xy], 2, []).'; ...
    result.pads(2).xy];
userXY = RectangularFpc.Geometry.Board_And_Coil('internal_to_user', internalXY, cfg);
...
for scanIndex = 1:numel(result.turnScan)
    ...
    if isfield(scan, 'failureCode')
        ...
        expectedCodes(scanIndex) = string(RectangularFpc.Export.Report_Artifacts('scan_failure_code', scan.failureReason));
    end

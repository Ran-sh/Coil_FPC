function varargout = ExportAll(operation, varargin)
...
switch operation
    case 'previews'
        [varargout{1:nargout}] = RectangularFpc.Export.PreviewExport( ...
            'write', varargin{:});
    case 'formal_export'
    ...
    case 'verify_output'
        ...
        RectangularFpc.Export.ExportVerification( ...
            'readable_artifact_set', outputFolder, cfg, result);
        RectangularFpc.Export.ExportIo('verify_file_manifest', fullfile( ...
...
outputRoot = RectangularFpc.Export.ExportIo( ...
...
RectangularFpc.Export.ExportIo('prepare_temp_folder', tempOutputFolder);
stagingCleanup = onCleanup(@() RectangularFpc.Export.ExportIo( ...
...
try
    ...
    RectangularFpc.Export.DxfArtifacts('write_board_outline', ...
    ...
    for layerIndex = 1:cfg.layerCount
        ...
        RectangularFpc.Export.DxfArtifacts('write_centerline', ...
    ...
    RectangularFpc.Export.ReportArtifacts('write_coordinates', fullfile( ...
        reportsFolder, '01_pad_via_coordinates.csv'), cfg, result);
    RectangularFpc.Export.ReportArtifacts('write_design_summary', fullfile( ...
        reportsFolder, '03_design_summary.txt'), cfg, result);
    RectangularFpc.Export.ReportArtifacts('write_turn_scan', fullfile( ...
        reportsFolder, '04_turn_scan.csv'), result);
    RectangularFpc.Export.ReportArtifacts('write_validation', fullfile( ...
        reportsFolder, '05_validation_report.txt'), result.validation);

    if cfg.enablePreview
        RectangularFpc.Export.PreviewExport( ...
    ...
    RectangularFpc.Export.ReportArtifacts('write_generation_status', ...
    ...
    RectangularFpc.Publish.PublishAtomically(tempOutputFolder, outputFolder);
catch ME
    rethrow(ME);
end
...
RectangularFpc.Export.ExportIo('copy_required_file', boardDxfFile, ...
    fullfile(dxfFolder, '00_board_outline.dxf'));
RectangularFpc.Export.DxfArtifacts('write_drill_map', ...
...
for layerIndex = 1:cfg.layerCount
    ...
    RectangularFpc.Export.ExportIo('copy_required_file', ...
        sourceCenterline, centerlineFile);
    RectangularFpc.Export.DxfArtifacts('write_physical_copper', ...
    ...
    RectangularFpc.Export.DxfArtifacts('write_antipad', antipadFile, ...
        sprintf('ANTIPAD_KEEP_OUT_L%d', layerIndex), vias, layerIndex);
end

RectangularFpc.Export.ReportArtifacts('write_layer_mapping', ...
    fullfile(reportsFolder, '02_layer_mapping.csv'), cfg);
RectangularFpc.Export.ReportArtifacts('write_manufacturing_check', ...
    fullfile(reportsFolder, '06_manufacturing_check.csv'), manufacturing);
RectangularFpc.Export.ReportArtifacts('write_fabrication_notes', ...
...
RectangularFpc.Export.ExportIo('write_file_manifest', manifestPath, outputFolder);
RectangularFpc.Export.ExportIo('verify_file_manifest', manifestPath, outputFolder);

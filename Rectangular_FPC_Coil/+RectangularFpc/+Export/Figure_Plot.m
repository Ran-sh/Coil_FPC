function fig = Figure_Plot(result)
%FIGURE_PLOT Internal figure viewer for the rectangular FPC runtime.
%   FIG = RectangularFpc.Export.Figure_Plot(RESULT) 是包内的内部实现，由
%   rectangular_fpc_main 在 cfg.enableFigure 为 true（默认）且运行于 MATLAB 桌面
%   环境时自动调用，不作为公共 API。包成员可以被限定名调用，但这里没有兼容性
%   承诺：请使用两个公开入口（rectangular_fpc_main /
%   rectangular_fpc_default_config），不要直接调用本函数。
%

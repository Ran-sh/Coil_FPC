function varargout = ReportArtifacts(operation, varargin)
...
fid = RectangularFpc.Export.ExportIo('open_utf8_file', filename, 'pad/via coordinate CSV');
...
for viaIndex = 1:numel(result.vias)
    ...
    xyUser = RectangularFpc.Geometry.BoardAndCoil('internal_to_user', via.xy, cfg);
    annularRing = (via.padDiameter - via.drillDiameter) / 2;
    fprintf(fid, '%s,%.6f,%.6f,%.6f,%.6f,L%d,L%d,%s,%s,%s,', ...
        RectangularFpc.Export.ExportIo('csv_text', via.name), xyUser(1), xyUser(2), via.xy(1), via.xy(2), ...
        via.fromLayer, via.toLayer, RectangularFpc.Export.ExportIo('csv_text', via.type), ...
        RectangularFpc.Export.ExportIo('csv_text', via.placementRegion), RectangularFpc.Export.ExportIo('csv_text', via.placementMode));
    fprintf(fid, '%.9f,%.9f,%.9f,%.9f,%s\n', ...
        via.padDiameter, via.drillDiameter, annularRing, ...
        via.antipadDiameter, RectangularFpc.Export.ExportIo('csv_text', via.role));
end
...
xyUser = RectangularFpc.Geometry.BoardAndCoil('internal_to_user', pad.xy, cfg);
fprintf(fid, '%s,%.6f,%.6f,%.6f,%.6f,L1,external,pad,', ...
    RectangularFpc.Export.ExportIo('csv_text', pad.name), xyUser(1), xyUser(2), pad.xy(1), pad.xy(2));
fprintf(fid, 'EXTERNAL_PAD,fixed,%.9f,0,0,0,%s\n', ...
    pad.diameter, RectangularFpc.Export.ExportIo('csv_text', description));
...
fid = RectangularFpc.Export.ExportIo('open_utf8_file', filename, 'design summary');
cleanup = onCleanup(@() fclose(fid));
...
fid = RectangularFpc.Export.ExportIo('open_utf8_file', filename, 'turn scan CSV');
cleanup = onCleanup(@() fclose(fid));
...
for scanIndex = 1:numel(result.turnScan)
    ...
    fprintf(fid, '%d,%d,%d,%s,%s\n', ...
        ...
        RectangularFpc.Export.ExportIo('csv_text', failureCode), RectangularFpc.Export.ExportIo('csv_text', scan.failureReason));
end
clear cleanup;
...
fid = RectangularFpc.Export.ExportIo('open_utf8_file', filename, 'validation report');
cleanup = onCleanup(@() fclose(fid));
...
fid = RectangularFpc.Export.ExportIo('open_utf8_file', filename, 'layer mapping CSV');
cleanup = onCleanup(@() fclose(fid));
...
fid = RectangularFpc.Export.ExportIo('open_utf8_file', filename, 'manufacturing check CSV');
cleanup = onCleanup(@() fclose(fid));
...
for checkIndex = 1:numel(manufacturing.checks)
    ...
    fprintf(fid, '%s,%s,%.17g,%.17g,%.17g,%s,%s\n', ...
        RectangularFpc.Export.ExportIo('csv_text', check.id), RectangularFpc.Export.ExportIo('csv_text', check.status), check.measuredMm, ...
        check.limitMm, check.marginMm, RectangularFpc.Export.ExportIo('csv_text', check.code), ...
        RectangularFpc.Export.ExportIo('csv_text', check.message));
end
clear cleanup;
...
fid = RectangularFpc.Export.ExportIo('open_utf8_file', filename, 'fabrication notes');
cleanup = onCleanup(@() fclose(fid));

...
for layerIndex = 1:cfg.layerCount
    ...
    result.layers(layerIndex).solidCopperDxfFile = fullfile(layerFolder, ...
        sprintf('%02d_copper_solid_L%d.dxf', layerIndex, layerIndex));
    result.layers(layerIndex).terminalSolidDxfFile = fullfile(layerFolder, ...
        sprintf('%02d_copper_solid_with_terminals_L%d.dxf', ...
...
for layerIndex = 1:cfg.layerCount
    ...
    % COMSOL-oriented closed copper outlines: the coil-only ring and the
    % layer's full conductor ring(s), both derived from the validated centerlines.
    solidFile = fullfile(layerFolder, sprintf( ...
        '%02d_copper_solid_L%d.dxf', layerIndex, layerIndex));
    terminalSolidFile = fullfile(layerFolder, sprintf( ...
        '%02d_copper_solid_with_terminals_L%d.dxf', layerIndex, layerIndex));
    RectangularFpc.Export.Dxf_Artifacts('write_solid_copper', solidFile, ...
        RectangularFpc.Geometry.Copper_Outline('coil_rings', ...
            result.layers(layerIndex).spiralXY, cfg.traceWidth), ...
        sprintf('COPPER_SOLID_L%d', layerIndex));
    RectangularFpc.Export.Dxf_Artifacts('write_terminal_solid_copper', ...
        terminalSolidFile, ...
        RectangularFpc.Geometry.Copper_Outline('conductor_rings', ...
            layerPaths{layerIndex}, cfg.traceWidth), ...
        sprintf('COPPER_SOLID_TERMINALS_L%d', layerIndex));
end

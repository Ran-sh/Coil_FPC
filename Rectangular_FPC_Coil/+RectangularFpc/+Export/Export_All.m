function varargout = Export_All(operation, varargin)
%EXPORT_ALL Private export dispatcher for the rectangular FPC runtime.
...
switch operation
    case 'previews'
        [varargout{1:nargout}] = RectangularFpc.Export.Preview_Export( ...
            'write', varargin{:});
    case 'formal_export'
    ...
    case 'verify_output'
        ...
        RectangularFpc.Export.Export_Verification( ...
            'readable_artifact_set', outputFolder, cfg, result);
        RectangularFpc.Export.Export_Io('verify_file_manifest', fullfile( ...
...
outputRoot = RectangularFpc.Export.Export_Io( ...
...
RectangularFpc.Export.Export_Io('prepare_temp_folder', tempOutputFolder);
stagingCleanup = onCleanup(@() RectangularFpc.Export.Export_Io( ...
...
try
    ...
    RectangularFpc.Export.Dxf_Artifacts('write_board_outline', ...
    ...
    for layerIndex = 1:cfg.layerCount
        ...
        RectangularFpc.Export.Dxf_Artifacts('write_centerline', ...
    ...
    RectangularFpc.Export.Report_Artifacts('write_coordinates', fullfile( ...
        reportsFolder, '01_pad_via_coordinates.csv'), cfg, result);
    RectangularFpc.Export.Report_Artifacts('write_design_summary', fullfile( ...
        reportsFolder, '03_design_summary.txt'), cfg, result);
    RectangularFpc.Export.Report_Artifacts('write_turn_scan', fullfile( ...
        reportsFolder, '04_turn_scan.csv'), result);
    RectangularFpc.Export.Report_Artifacts('write_validation', fullfile( ...
        reportsFolder, '05_validation_report.txt'), result.validation);

    if cfg.enablePreview
        RectangularFpc.Export.Preview_Export( ...
    ...
    RectangularFpc.Export.Report_Artifacts('write_generation_status', ...
    ...
    RectangularFpc.Publish.Publish_Atomically(tempOutputFolder, outputFolder);
catch ME
    rethrow(ME);
end
...
RectangularFpc.Export.Export_Io('copy_required_file', boardDxfFile, ...
    fullfile(dxfFolder, '00_board_outline.dxf'));
RectangularFpc.Export.Dxf_Artifacts('write_drill_map', ...
...
for layerIndex = 1:cfg.layerCount
    ...
    RectangularFpc.Export.Export_Io('copy_required_file', ...
        sourceCenterline, centerlineFile);
    RectangularFpc.Export.Dxf_Artifacts('write_physical_copper', ...
    ...
    RectangularFpc.Export.Dxf_Artifacts('write_antipad', antipadFile, ...
        sprintf('ANTIPAD_KEEP_OUT_L%d', layerIndex), vias, layerIndex);
end

RectangularFpc.Export.Report_Artifacts('write_layer_mapping', ...
    fullfile(reportsFolder, '02_layer_mapping.csv'), cfg);
RectangularFpc.Export.Report_Artifacts('write_manufacturing_check', ...
    fullfile(reportsFolder, '06_manufacturing_check.csv'), manufacturing);
RectangularFpc.Export.Report_Artifacts('write_fabrication_notes', ...
...
RectangularFpc.Export.Export_Io('write_file_manifest', manifestPath, outputFolder);
RectangularFpc.Export.Export_Io('verify_file_manifest', manifestPath, outputFolder);

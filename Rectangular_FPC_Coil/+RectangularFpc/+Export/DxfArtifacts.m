function DxfArtifacts(operation, varargin)
...
fid = RectangularFpc.Export.ExportIo('open_ascii_file', filename, 'board-outline DXF');
...
fid = RectangularFpc.Export.ExportIo('open_ascii_file', filename, 'centerline DXF');
cleanup = onCleanup(@() fclose(fid));
writeDxfHeader(fid, layerName);
...
fid = RectangularFpc.Export.ExportIo('open_ascii_file', filename, 'DXF');
cleanup = onCleanup(@() fclose(fid));
writeDxfHeader(fid, layerName);

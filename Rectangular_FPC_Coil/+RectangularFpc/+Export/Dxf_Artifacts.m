function Dxf_Artifacts(operation, varargin)
%DXF_ARTIFACTS Write and verify DXF manufacturing artifacts.
...
fid = RectangularFpc.Export.Export_Io('open_ascii_file', filename, 'board-outline DXF');
...
fid = RectangularFpc.Export.Export_Io('open_ascii_file', filename, 'centerline DXF');
cleanup = onCleanup(@() fclose(fid));
writeDxfHeader(fid, layerName);

function ExportVerification(operation, varargin)
...
RectangularFpc.Export.DxfArtifacts('verify_all', outputFolder, cfg, result);
...
expectedSvgNames = RectangularFpc.Export.PreviewExport('expected_names', cfg);
...
internalXY = [result.pads(1).xy; reshape([result.vias.xy], 2, []).'; ...
    result.pads(2).xy];
userXY = RectangularFpc.Geometry.BoardAndCoil('internal_to_user', internalXY, cfg);
...
for scanIndex = 1:numel(result.turnScan)
    ...
    if isfield(scan, 'failureCode')
        ...
        expectedCodes(scanIndex) = string(RectangularFpc.Export.ReportArtifacts('scan_failure_code', scan.failureReason));
    end

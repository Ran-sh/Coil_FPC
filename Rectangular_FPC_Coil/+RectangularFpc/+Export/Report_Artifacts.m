function varargout = Report_Artifacts(operation, varargin)
%REPORT_ARTIFACTS Write textual and tabular export reports.
...
fid = RectangularFpc.Export.Export_Io('open_utf8_file', filename, 'pad/via coordinate CSV');
...
for viaIndex = 1:numel(result.vias)
    ...
    xyUser = RectangularFpc.Geometry.Board_And_Coil('internal_to_user', via.xy, cfg);
    annularRing = (via.padDiameter - via.drillDiameter) / 2;
    fprintf(fid, '%s,%.6f,%.6f,%.6f,%.6f,L%d,L%d,%s,%s,%s,', ...
        RectangularFpc.Export.Export_Io('csv_text', via.name), xyUser(1), xyUser(2), via.xy(1), via.xy(2), ...
        via.fromLayer, via.toLayer, RectangularFpc.Export.Export_Io('csv_text', via.type), ...
        RectangularFpc.Export.Export_Io('csv_text', via.placementRegion), RectangularFpc.Export.Export_Io('csv_text', via.placementMode));
    fprintf(fid, '%.9f,%.9f,%.9f,%.9f,%s\n', ...
        via.padDiameter, via.drillDiameter, annularRing, ...
        via.antipadDiameter, RectangularFpc.Export.Export_Io('csv_text', via.role));
end
...
xyUser = RectangularFpc.Geometry.Board_And_Coil('internal_to_user', pad.xy, cfg);
fprintf(fid, '%s,%.6f,%.6f,%.6f,%.6f,L1,external,pad,', ...
    RectangularFpc.Export.Export_Io('csv_text', pad.name), xyUser(1), xyUser(2), pad.xy(1), pad.xy(2));
fprintf(fid, 'EXTERNAL_PAD,fixed,%.9f,0,0,0,%s\n', ...
    pad.diameter, RectangularFpc.Export.Export_Io('csv_text', description));
...
fid = RectangularFpc.Export.Export_Io('open_utf8_file', filename, 'design summary');
cleanup = onCleanup(@() fclose(fid));
...
fid = RectangularFpc.Export.Export_Io('open_utf8_file', filename, 'turn scan CSV');
cleanup = onCleanup(@() fclose(fid));
...
for scanIndex = 1:numel(result.turnScan)
    ...
    fprintf(fid, '%d,%d,%d,%s,%s\n', ...
        ...
        RectangularFpc.Export.Export_Io('csv_text', failureCode), RectangularFpc.Export.Export_Io('csv_text', scan.failureReason));
end
clear cleanup;
...
fid = RectangularFpc.Export.Export_Io('open_utf8_file', filename, 'validation report');
cleanup = onCleanup(@() fclose(fid));
...
fid = RectangularFpc.Export.Export_Io('open_utf8_file', filename, 'layer mapping CSV');
cleanup = onCleanup(@() fclose(fid));
...
fid = RectangularFpc.Export.Export_Io('open_utf8_file', filename, 'manufacturing check CSV');
cleanup = onCleanup(@() fclose(fid));
...
for checkIndex = 1:numel(manufacturing.checks)
    ...
    fprintf(fid, '%s,%s,%.17g,%.17g,%.17g,%s,%s\n', ...
        RectangularFpc.Export.Export_Io('csv_text', check.id), RectangularFpc.Export.Export_Io('csv_text', check.status), check.measuredMm, ...
        check.limitMm, check.marginMm, RectangularFpc.Export.Export_Io('csv_text', check.code), ...
        RectangularFpc.Export.Export_Io('csv_text', check.message));
end
clear cleanup;
...
fid = RectangularFpc.Export.Export_Io('open_utf8_file', filename, 'fabrication notes');
cleanup = onCleanup(@() fclose(fid));
...
fprintf(fid, ['Publication read protocol: call ', ...
    'RectangularFpc.Publish.Read_Committed(outputPath, reader) so the complete ', ...
...
fprintf(fid, ['COMSOL solid geometry: dxf/Ln/NN_copper_solid_Ln.dxf holds only the ', ...
    'layer''s main spiral as one closed ring; ', ...
    'dxf/Ln/NN_copper_solid_with_terminals_Ln.dxf holds that layer''s full ', ...
    'conductor centerline (spiral plus its escape, via, and terminal leads) as ', ...
    'one closed ring per connected island, so L1 carries two rings (series ', ...
    'chain and VOUT to PAD_B return) and every other layer carries one.\n']);
fprintf(fid, ['COMSOL solid contents: neither solid file contains electrode pads, ', ...
    'via pads, drills, or antipad keepouts; the ends are straight caps at the ', ...
    'pad and via centres. Both are closed widthless LWPOLYLINEs and are never ', ...
    'split at maxVerticesPerDxfEntity, because a split ring is no longer a ', ...
    'closed domain boundary.\n']);
fprintf(fid, ['COMSOL layer height: this export is XY geometry only. Take layer Z ', ...
    'positions from the fabricator''s stackup; the selected profile does not ', ...
    'verify layer counts above 4, so no copper thickness position is implied ', ...
    'here.\n']);
clear cleanup;

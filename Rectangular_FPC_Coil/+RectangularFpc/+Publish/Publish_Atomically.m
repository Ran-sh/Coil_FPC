function varargout = Publish_Atomically( ...
...
if nargin >= 1 && ischar(tempOutputFolder) && ...
    ...
    outputFolder = RectangularFpc.Publish.Publish_Paths( ...
        'validate_access_target', outputFolder);
    [varargout{1}, ~] = RectangularFpc.Publish.Publish_Lock( ...
...
if nargin >= 1 && ischar(tempOutputFolder) && ...
    ...
    varargout{1} = RectangularFpc.Publish.Commit_Contract(outputFolder);
    return;
end
...
[tempOutputFolder, outputFolder] = RectangularFpc.Publish.Publish_Paths( ...
...
[lockCleanup, lockToken] = RectangularFpc.Publish.Publish_Lock( ...
    'acquire', lockFolder, moveFolder); %#ok<ASGLU>

RectangularFpc.Publish.Backup_Recovery( ...
    'recover', outputFolder, moveFolder, lockFolder, lockToken);
[stagingCommitted, stagingReason] = ...
    RectangularFpc.Publish.Commit_Contract(tempOutputFolder);
if ~stagingCommitted
...
transactionToken = RectangularFpc.Publish.Publish_Lock('new_token');
backupFolder = sprintf('%s_backup_%s', outputFolder, transactionToken);
backupMarker = RectangularFpc.Publish.Backup_Recovery( ...
...
try
    if hadPriorVersion
        RectangularFpc.Publish.Publish_Lock( ...
            'assert_owned', lockFolder, lockToken);
        backupMarker = RectangularFpc.Publish.Backup_Recovery( ...
            'write_marker', backupFolder, outputFolder, transactionToken);
        RectangularFpc.Publish.Publish_Lock( ...
            'assert_owned', lockFolder, lockToken);
        [moved, message, moveClean] = RectangularFpc.Publish.Publish_Lock( ...
    ...
    [publishedCommitted, publishedReason] = ...
        RectangularFpc.Publish.Commit_Contract(outputFolder);
...
if priorVersionMoved && isfolder(backupFolder)
    if isempty(cleanupFailure)
        [restored, restoreMessage] = RectangularFpc.Publish.Publish_Lock( ...

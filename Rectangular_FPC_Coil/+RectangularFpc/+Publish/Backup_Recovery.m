function varargout = Backup_Recovery(action, varargin)
%BACKUP_RECOVERY Target-bound publication backup protocol.
...
if isfolder(outputFolder)
    [committed, reason] = RectangularFpc.Publish.Commit_Contract(outputFolder);
    if ~committed
    ...
    if ~isempty(state.validFolders)
        for backupIndex = 1:numel(state.validFolders)
            RectangularFpc.Publish.Publish_Lock('remove_folder', ...
                state.validFolders{backupIndex}, lockFolder, lockToken);
            RectangularFpc.Publish.Publish_Lock('delete_file', ...
...
[backupCommitted, backupReason] = ...
    RectangularFpc.Publish.Commit_Contract(backupFolder);
if ~backupCommitted
    error('RectangularFPC:AtomicRecoveryFailed', ...
        'Recovery backup is not committed: %s (%s)', ...
        backupFolder, backupReason);
end
[restored, message, moveClean] = RectangularFpc.Publish.Publish_Lock( ...
...
for entryIndex = 1:numel(entries)
    ...
    if markerValid
        backupCommitted = RectangularFpc.Publish.Commit_Contract(backupFolder);
    end
...
path = RectangularFpc.Publish.Publish_Paths('normalize', path);

function varargout = BackupRecovery(action, varargin)
...
if isfolder(outputFolder)
    [committed, reason] = RectangularFpc.Publish.CommitContract(outputFolder);
    if ~committed
    ...
    if ~isempty(state.validFolders)
        for backupIndex = 1:numel(state.validFolders)
            RectangularFpc.Publish.PublishLock('remove_folder', ...
                state.validFolders{backupIndex}, lockFolder, lockToken);
            RectangularFpc.Publish.PublishLock('delete_file', ...
...
[backupCommitted, backupReason] = ...
    RectangularFpc.Publish.CommitContract(backupFolder);
if ~backupCommitted
    error('RectangularFPC:AtomicRecoveryFailed', ...
        'Recovery backup is not committed: %s (%s)', ...
        backupFolder, backupReason);
end
[restored, message, moveClean] = RectangularFpc.Publish.PublishLock( ...
...
for entryIndex = 1:numel(entries)
    ...
    if markerValid
        backupCommitted = RectangularFpc.Publish.CommitContract(backupFolder);
    end
...
path = RectangularFpc.Publish.PublishPaths('normalize', path);

function varargout = Read_Committed(outputFolder, reader)
%READ_COMMITTED Read one published version under its lock.
%   VALUE = READ_COMMITTED(OUTPUTFOLDER, READER) invokes
...
lockCleanup = RectangularFpc.Publish.Publish_Atomically( ...
...
if ~RectangularFpc.Publish.Publish_Atomically('verify_committed', outputFolder)

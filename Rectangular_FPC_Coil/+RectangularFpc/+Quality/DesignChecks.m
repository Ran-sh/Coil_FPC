function varargout = DesignChecks(operation, varargin)
...
allPaths = RectangularFpc.Geometry.PathGeometry('flatten_layers', layerPaths);
if any(~isfinite(boardXY), 'all') || ...
        any(cellfun(@(path) any(~isfinite(path), 'all'), allPaths))
    nanInfPass = false;
    failures{end+1} = '存在NaN或Inf坐标';
end
if RectangularFpc.Geometry.BoardAndCoil('has_zero_length', boardXY, tol) || ...
        any(cellfun(@(path) RectangularFpc.Geometry.BoardAndCoil( ...
...
[topologyPass, topologyIssues, topologyEndpointErrors] = ...
    RectangularFpc.Quality.ValidationReport('electrical_topology', cfg, d, layerPaths, vias);
if ~topologyPass
...
if cfg.enableBoardAngleCheck
    boardMinAngle = RectangularFpc.Geometry.PathGeometry('minimum_closed_angle', boardXY, tol);
...
if cfg.enableCopperAngleCheck
    for layerIndex = 1:cfg.layerCount
        pathAngles = cellfun(@(path) ...
            RectangularFpc.Geometry.PathGeometry('minimum_open_angle', path, tol), ...
...
if cfg.enableExactSelfIntersectionCheck
    if RectangularFpc.Geometry.PathGeometry('self_intersection', boardXY, true, cfg)
        boardSelfIntersectionPass = false;
        failures{end+1} = '板框存在自相交';
    end
    for layerIndex = 1:cfg.layerCount
        for pathIndex = 1:numel(layerPaths{layerIndex})
            if RectangularFpc.Geometry.PathGeometry('self_intersection',  ...
        ...
        for pathA = 1:numel(layerPaths{layerIndex})-1
            for pathB = pathA+1:numel(layerPaths{layerIndex})
                if RectangularFpc.Geometry.PathGeometry('minimum_distance_polylines',  ...
    ...
    for layerIndex = 1:cfg.layerCount
        ...
        for pathIndex = 1:numel(layerPaths{layerIndex})
            [pathDistance, pathPass] = ...
                RectangularFpc.Geometry.PathGeometry('minimum_nonadjacent_distance',  ...
        ...
        for pathA = 1:numel(layerPaths{layerIndex})-1
            for pathB = pathA+1:numel(layerPaths{layerIndex})
                pathDistance = RectangularFpc.Geometry.PathGeometry('minimum_distance_polylines',  ...
                    layerPaths{layerIndex}{pathA}, ...
...
manufacturingRules = RectangularFpc.Quality.JlcRules('resolve', cfg).rules;
...
reportLines = RectangularFpc.Quality.ValidationReport('build',  ...
...
pass = (inA || onA) && (inB || onB) && ...
       RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', padA, boardXY) >= radius - tol && ...
       RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', padB, boardXY) >= radius - tol;
...
for layerIndex = 1:numel(layerPaths)
    for pathIndex = 1:numel(layerPaths{layerIndex})
        ...
        if layerIndex == 1 && pathIndex == 1
            dA = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline_excluding',  ...
                padA, path, true, connectedExcludedLength);
        else
            dA = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', padA, path);
        end
        ...
        if layerIndex == 1 && pathIndex == 2
            ...
            dB = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', padB, path);
        end
...
for k = 1:numel(vias)
    ...
    d = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', vias(k).xy, boardXY);
...
for k = 1:numel(vias)
    ...
    for layerIndex = 1:numel(layerPaths)
        for pathIndex = 1:numel(layerPaths{layerIndex})
            ...
            if isFromPath
                d = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline_excluding',  ...
            ...
            elseif isToPath
                ...
                if checksThisLayer
                    ...
                    d = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', vias(k).xy, path);
...
for pathIndex = 1:numel(allPaths)
    ...
    minCopperToBoard = min(minCopperToBoard, ...
        RectangularFpc.Geometry.PathGeometry('minimum_distance_polylines', path, boardXY) - halfTrace);
end
minCopperToBoard = min(minCopperToBoard, ...
    RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', d.padA, boardXY) - cfg.padDiameter / 2);
minCopperToBoard = min(minCopperToBoard, ...
    RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', d.padB, boardXY) - cfg.padDiameter / 2);
for viaIndex = 1:numel(vias)
    minCopperToBoard = min(minCopperToBoard, ...
        RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', vias(viaIndex).xy, boardXY) - ...
...
for viaIndex = 1:numel(vias)
    ...
    for layerIndex = connectedLayers
        for pathIndex = 1:numel(layerPaths{layerIndex})
            ...
            if isFromPath
                centerDistance = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline_excluding',  ...
            ...
            elseif isToPath
                ...
                centerDistance = RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline', via.xy, path);
            end
...
for viaIndex = 1:numel(vias)
    ...
    for layerIndex = setdiff(1:numel(layerPaths), connectedLayers)
        for pathIndex = 1:numel(layerPaths{layerIndex})
            path = layerPaths{layerIndex}{pathIndex};
            dMin = min(dMin, RectangularFpc.Geometry.PathGeometry('minimum_distance_point_polyline',  ...

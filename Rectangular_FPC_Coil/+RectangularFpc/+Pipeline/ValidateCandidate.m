function varargout = ValidateCandidate(operation, varargin)
...
candidatePaths = RectangularFpc.Geometry.PathGeometry('flatten_layers', layerPaths);
if any(~isfinite(boardXY), 'all') || ...
        any(cellfun(@(xy) any(~isfinite(xy), 'all'), candidatePaths)) || ...
        any(cellfun(@(xy) RectangularFpc.Geometry.BoardAndCoil( ...
...
for k = 1:cfg.layerCount
    ...
    for pathIndex = 1:numel(layerPaths{k})
        ...
        if cfg.enableCopperAngleCheck && ...
            ...
            minAng = RectangularFpc.Geometry.PathGeometry('minimum_open_angle', path, tol);
            reason = sprintf('L%d路径%d铜线角度不足（最小内角 %.3f°）', ...
                k, pathIndex, minAng);
            return;
        end
        if cfg.enableExactSelfIntersectionCheck && ...
                RectangularFpc.Geometry.PathGeometry('self_intersection', path, false, cfg)
            reason = sprintf('L%d路径%d存在自相交', k, pathIndex);
            return;
        end
        if cfg.enableCopperClearanceCheck
            [~, spacingPass] = RectangularFpc.Geometry.PathGeometry('minimum_nonadjacent_distance',  ...
...
if cfg.enablePadClearanceCheck
    if ~RectangularFpc.Quality.DesignChecks('pad_to_board', d.padA, d.padB, closedBoardXY, cfg, tol)
        reason = '焊盘未完整位于板框内';
        return;
    end
    if ~RectangularFpc.Quality.DesignChecks('pad_to_pad', d.padA, d.padB, cfg, tol)
        reason = '焊盘间距不足';
        return;
    end
    if ~RectangularFpc.Quality.DesignChecks('pad_to_copper',  ...
...
if cfg.enableViaClearanceCheck
    if ~RectangularFpc.Quality.DesignChecks('via_to_via', viaXY, cfg, tol)
        reason = '过孔间距不足';
        return;
    end
    if ~RectangularFpc.Quality.DesignChecks('via_to_board', vias, closedBoardXY, cfg, tol)
        reason = '过孔到板框间距不足';
        return;
    end
    if ~RectangularFpc.Quality.DesignChecks('via_to_pad', viaXY, d.padA, d.padB, cfg, tol)
        reason = '过孔到焊盘间距不足';
        return;
    end
    ...
    [connectedPass, nonConnectedPass] = RectangularFpc.Quality.DesignChecks('via_to_copper',  ...

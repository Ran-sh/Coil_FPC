...
- 6 / 8 层改用与 2 / 4 层相同的贯穿通孔模型（连接层焊盘 + 非连接层反焊盘）：跨层净距检查、引线反焊盘避让与 `VIA_TECHNOLOGY` 判定随之生效，该行由 WARN 变为 PASS；尾板过孔行在 6 / 8 层自动抬高到「反焊盘半径 + 半线宽 + 外侧逃逸弧半径」以上（默认 0.5 → 0.9 mm）以让出引线通道。未验证状态只剩「JLCPCB FPC 能力清单不含 6 / 8 层」这一条厂家事实，2 / 4 层几何与资格不变。

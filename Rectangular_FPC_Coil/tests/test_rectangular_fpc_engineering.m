...
function testCollinearSeriesViasFailQualifiedManufacturing(testCase)
% VIA_STAGGER 规则（原手动坐标测试的替代）：自动放置的串联过孔必须错开；
% 把串联过孔改成共线（旧手动模式能造出、自动放置不再产生的退化输入）后，
% 制造检查必须给出 FAIL / VIA_ROW_COLLINEAR 并撤销制造资格与导出许可。
% 通过公开质量入口复核：检查只读入参 vias/度量，不重新生成几何。
result = rectangular_fpc_main(struct( ...
    'analysisOnly', true, ...
    'layerCount', 4, ...
    'turnsPerLayer', 1, ...
...
verifyEqual(testCase, stagger.status, 'PASS');
vias = result.vias;
seriesIdx = find(strcmp({vias.role}, 'series_interconnect'));
verifyGreaterThanOrEqual(testCase, numel(seriesIdx), 3);
commonY = vias(seriesIdx(1)).xy(2);
for k = seriesIdx(:).'
    vias(k).xy(2) = commonY;
end
degenerate = RectangularFpc.Quality.Jlc_Rules('check_result', result.config, struct( ...
    'vias', vias, ...
    ...
    'minDrillToNonConnectedCopperMm', result.validation.minDrillToNonConnectedCopperMm, ...
    'minDrillToBoardMm', result.validation.minDrillToBoardMm, ...
    'minViaToBoardMm', result.validation.minViaToBoardMm, ...
    'viaNonConnectedCopperPassed', result.validation.viaNonConnectedCopperPassed, ...
    'topologyPassed', result.validation.topologyPassed));
stagger = degenerate.checks(strcmp({degenerate.checks.id}, 'VIA_STAGGER'));
assertNumElements(testCase, stagger, 1);
verifyEqual(testCase, stagger.status, 'FAIL');
verifyEqual(testCase, stagger.code, 'VIA_ROW_COLLINEAR');
verifyFalse(testCase, degenerate.verified);
verifyFalse(testCase, degenerate.exportAllowed);
end
...
verifyEqual(testCase, standard.manufacturing.requestedProfile, ...
    'jlc_fpc_1oz');
verifyEqual(testCase, standard.manufacturing.sourceCheckedOn, '2026-09-05');

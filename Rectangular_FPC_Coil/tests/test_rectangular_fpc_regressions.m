...
expectedPathHashes = { ...
    'b24996b4a8ec4d8aaf8ae5971e22932f90e635098627091fe0ad6f57f1d7acec'; ...
    '8fc4ae9d45046bc503a64d17c95ab2b0f3827d4b159ecd7763d7a08914e9147a'; ...
    'e204f93172d8e3ed108fdbe8fc715a2a22a110ff62eaf0c05f9622c11c180590'; ...
    'b0a511d93ee90df565da01a229d8d5d95b8415a36c353e85c98104fbe56e4ac9'};
...
verifyEqual(testCase, vertcat(result.vias.xy), [ ...
    34.34, 0.0; 40.80, 0.50; 32.34, 0.0; 40.80, -1.10], ...
    'AbsTol', cfg.geometryTolerance);
...
% or nested under MATLAB's rules, so helpers live in a module-specific package
% tree grouped by function. Assert the tree by walking it rather than pinning a
% flat name list, so adding a helper in the right group does not fail the test
% while a helper dropped outside the tree still does.
...
for k = 1:numel(groups)
    ...
    verifyEqual(testCase, isfolder(g), true, ...
        sprintf('+RectangularFpc/%s must exist', groups{k}));
end
helperFiles = dir(fullfile(pkgRoot, '**', '*.m'));
verifyGreaterThan(testCase, numel(helperFiles), 0, ...
    'the helper package must not be empty');
for k = 1:numel(helperFiles)
    % every helper's function declaration must match its filename, otherwise a
    % package-qualified call resolves to the wrong name.
    fn = regexprep(helperFiles(k).name, '\.m$', '');
    dec = regexp(fileread(fullfile(helperFiles(k).folder, helperFiles(k).name)), ...
        '^function\s+(?:\[[^\]]*\]\s*=\s*|varargout\s*=\s*|\w+\s*=\s*)?([A-Za-z_]\w*)', ...
        'tokens', 'once');
    verifyFalse(testCase, isempty(dec), ...
        sprintf('%s has no function declaration', helperFiles(k).name));
    verifyEqual(testCase, dec{1}, fn, ...
        sprintf('%s declares function %s', helperFiles(k).name, dec{1}));
end
...
% 负偏移（板中线另一侧）严格保留方向、不静默镜像；6/8 层该侧容不下反焊盘
% 引线通道时必须 fail closed（实测负侧不可行），而不是把行搬到正侧。
for layerCount = [6, 8]
    verifyError(testCase, @() rectangular_fpc_main(struct( ...
        'layerCount', layerCount, ...
        'turnsPerLayer', 1, ...
        'outerViaRowOffsetY', -0.5, ...
        'analysisOnly', true, ...
        'enablePreview', false, ...
        'enableFigure', false)), 'RectangularFPC:ViaPlanningFailed');
end
% 2/4 层没有该下界要求，负偏移仍按既有"偏好值"语义工作：默认 4 层几何下
% 规划器把该行放在负侧（实测 -0.25 mm），不得被镜像到正侧。
fourLayerNegative = rectangular_fpc_main(struct( ...
    'layerCount', 4, ...
    'turnsPerLayer', 1, ...
    'outerViaRowOffsetY', -0.5, ...
    'analysisOnly', true, ...
    'enablePreview', false, ...
    'enableFigure', false));
verifyTrue(testCase, fourLayerNegative.passed);
fourLayerTab = fourLayerNegative.vias(strcmp( ...
    {fourLayerNegative.vias.placementRegion}, 'RIGHT_TAB'));
verifyNotEmpty(testCase, fourLayerTab);
fourLayerRow = vertcat(fourLayerTab.xy);
verifyLessThan(testCase, max(fourLayerRow(:, 2)), 0, ...
    '4-layer negative offset must stay on the negative side.');
end

function testLegacyAutoIsRejectedAboveTwoLayers(testCase)
% legacy_auto 把同奇偶的串联过孔放在同一锚点：3 个以上过孔在贯穿通孔模型下
% 会重合短路串联链，且改动前 4/6/8 层本就无法通过布线/匝数扫描。现在对
% >2 层显式拒绝，2 层（唯一 1 个串联过孔）保持可用。
for layerCount = [4, 6, 8]
    verifyError(testCase, @() rectangular_fpc_main(struct( ...
        'layerCount', layerCount, 'turnsPerLayer', 1, ...
        'viaPlacementMode', 'legacy_auto', 'analysisOnly', true, ...
        'enablePreview', false, 'enableFigure', false)), ...
        'RectangularFPC:InvalidConfigValue');
end
legacyTwoLayer = rectangular_fpc_main(struct( ...
    'layerCount', 2, 'turnsPerLayer', 1, 'viaPlacementMode', 'legacy_auto', ...
    'analysisOnly', true, 'enablePreview', false, 'enableFigure', false));
verifyTrue(testCase, legacyTwoLayer.passed);
end

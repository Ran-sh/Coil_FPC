...
if fullyValidatedMaxTurns < 1
    % The turn scan records compact failure text so it can continue from a
    % high candidate to a lower one. If no candidate survives, rebuild the
    % caller-requested geometry once so hard routing/planning failures keep
    % their actionable public identifiers instead of becoming a generic
    % NoValidTurnCount error.
    try
        [~, ~, ~, ~, ~] = rectangular_fpc_geometry('build_layers', ...
...
manufacturingInput = struct( ...
    ...
    'minViaToBoardMm', validation.minViaToBoardMm, ...
    'vias', vias, ...

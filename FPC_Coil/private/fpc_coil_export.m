function varargout = fpc_coil_export(operation, varargin)
%FPC_COIL_EXPORT Private export dispatcher for the FPC runtime.
...
switch operation
    ...
    otherwise
        error('FPC_Coil:UnknownExportOperation', ...
...
for v = 1:numel(vias)
    ...
    if via.antipadDiameter > 0
        drawCircle(via.xy(1), via.xy(2), via.antipadDiameter, [0.6 0.2 0.2], 1.0, 'VOUT antipad', '--');
    end
...
text(cfg.plateLength/2 - 7.5, cfg.plateWidth/2 - 0.7, ...
    'Outer: via pad | Inner: drill | Dashed: VOUT antipad', ...
...
% Standalone preview for one copper layer: only that layer's paths, pads on
% L1 only, and vias whose connectedLayers contains this layer.
...
for v = 1:numel(vias)
    if ~any(vias(v).connectedLayers == k)
        continue;
    end
    drawCircle(vias(v).xy(1), vias(v).xy(2), vias(v).padDiameter, [0.2 0.35 0.6], 1.2, vias(v).name);
    drawCircle(vias(v).xy(1), vias(v).xy(2), vias(v).drillDiameter, 'k', 0.8);
end

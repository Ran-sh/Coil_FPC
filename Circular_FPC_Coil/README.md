...
同根下的旧**完整产物**（含 `reports/08_file_manifest.csv` 的目录）移入 `archive/`，
并把 `LATEST.txt` 指向最新产物；归档目标重名时追加 `_2`、`_3`…… 后缀，绝不覆盖。
非完整目录（临时目录等）保持原位；单个目录移动失败只告警、不回滚已发布的新产物。
要并排保留多份产物（如 `examples/generate_all_variants.m` 批量生成六种层叠），把该
开关设为 `false`。

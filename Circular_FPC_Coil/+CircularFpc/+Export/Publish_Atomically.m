...
if isfolder(lockFolder)
    ...
    if ~ownerChanged && ~strcmp(readLockToken(claimDir), claimToken)
...
if ok
    ...
    try
        writeLockOwner(claimDir, claimToken);
    catch
...
if isfolder(claimDir) && strcmp(readLockToken(claimDir), token)
    rmdir(claimDir, 's');
end
end

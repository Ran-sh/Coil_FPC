...
- 铜线内角必须严格大于 90°，且默认禁止逃逸线回退为 90°尖角。
...
采用“铜线角度严格大于 90°且不得回退为尖角”的新规则后，两层默认完整上限为 10 匝、推荐 9 匝；11 匝仍满足宽度公式，但无法通过严格圆弧验证。板宽、线宽、线距、中心保留宽度或逃逸几何变化后，上限会重新计算。
...
| `requireSmoothEscapeArcs` | `true` | 禁止逃逸线回退为 90°尖角 |
...
- 默认要求所有逃逸连接成功生成平滑圆弧；如果只能形成 90°尖角，该候选匝数直接判定失败，不会进入正式输出。
